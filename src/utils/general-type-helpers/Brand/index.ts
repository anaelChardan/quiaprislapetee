...
type AcceptableBrandBases = string | number | boolean;

// From https://github.com/kourge/ts-brand
// We needed to copy in order to fix a type issue
/**
...
/**
 * This function create a Zod schema, that can be later used with `.safeParse`
 *
 * It requires a `BrandType` generic, and the `primitive` parameter is optional
 * since `string` will be set by default
 */
export const brandedTypeSchema = <
  Base extends AcceptableBrandBases,
  B extends Brand<Base, unknown>,
>(
  primitive: ToZodPrimitive<Base>,
): z.Schema<B> => z.any().refine((value) => z[primitive]().safeParse(value).success);

type ToZodPrimitive<T extends AcceptableBrandBases> = T extends string
  ? 'string'
  : T extends number
    ? 'number'
    : T extends boolean
